...
Returns a [`quote()`](https://rdrr.io/r/base/substitute.html)d filter
expression (for `get_data(file, filter = ...)`, same convention as any
other manual filter) that keeps only species in taxonomic groups
`rtrees::get_tree()` can currently build a phylogeny for: plants, and
among animals, birds, mammals, amphibians, reptiles, cartilaginous fish,
bony fish, bees, and butterflies. See `rtrees::taxa_supported()` for the
authoritative, currently-installed group list – this filter should be
kept in sync with it by hand, since it can't be queried from inside a
[`quote()`](https://rdrr.io/r/base/substitute.html)d expression.

## Usage

``` r
rtrees_supported_filter()
```

## Value

A [`quote()`](https://rdrr.io/r/base/substitute.html)d filter
expression, meant to be passed directly:
