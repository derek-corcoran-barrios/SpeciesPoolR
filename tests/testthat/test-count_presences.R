...
test_that("GBIF facet counts are parsed from named facet data frames", {
  facets <- list(species = data.frame(
    species = c("Trifolium repens", "Vicia cracca"),
    count = c(12, 3)
  ))

  out <- SpeciesPoolR:::extract_gbif_facet_counts(facets, facet = "species")

  expect_equal(out$species, c("Trifolium repens", "Vicia cracca"))
  expect_equal(out$N, c(12L, 3L))
})

test_that("GBIF facet counts are parsed from raw facet count lists", {
  facets <- list(list(
    field = "SPECIES",
    counts = list(list(name = "Trifolium repens", count = 12))
  ))
